...
static t_class *mind_fft_class;

typedef struct _mind_fft{
	t_object x_obj;
	t_outlet *out;
	t_outlet *img;
...
static void mind_fft_float(t_mind_fft *x, t_float f){
...
void *mind_fft_thread(void *arg){
	t_mind_fft *x = (t_mind_fft *) arg;
...
void *mind_fft_new(t_floatarg f){
	 
	t_mind_fft *x = (t_mind_fft*) pd_new(mind_fft_class);
	...
	pthread_create(&x->con,NULL,mind_fft_thread,x);
	return (void *)x;
}

void mind_fft_destroy(t_mind_fft *x){
...
void mind_fft_setup(void){
	mind_fft_class = class_new(gensym("mind_fft"), (t_newmethod) mind_fft_new,(t_method) mind_fft_destroy, sizeof(t_mind_fft),CLASS_DEFAULT,A_DEFFLOAT,0);
	class_addfloat(mind_fft_class,mind_fft_float);
}
